...
module.exports = function (spacing, colors) {
  if (!colors) {
    console.warn('default-shapes-builder: parameter colors is omited');
    colors = {}; // Hack. It should be removed.
  }
  if (typeof colors.isDarkTheme !== 'boolean') {
    console.warn(
      `default-shapes-builder: colors.isDarkTheme is not 'boolean' (colors.isDarkTheme = '${colors.isDarkTheme}')`
    );
  }

  return {
    isDarkTheme: colors ? colors.isDarkTheme : false,
    ...
    flyingShadow: colors.isDarkTheme
      ? '0px 10px 20px 8px rgba(0, 0, 0, 0.9)'
