...
const {color} = require('xcraft-core-converters');
const ColorConverters = color;
...
module.exports = function (colors) {
  if (!colors) {
    console.warn('default-palette-builder: parameter colors is omited');
  } else if (typeof colors.isDarkTheme !== 'boolean') {
    console.warn(
      `default-palette-builder: colors.isDarkTheme is not 'boolean' (colors.isDarkTheme = '${colors.isDarkTheme}')`
    );
  }

  Object.keys(colors).forEach((k) => {
    const color = colors[k];
    if (ColorConverters.check(color)) {
      const colorRGB = ColorConverters.toRGB(color);
      if (colorRGB !== color) {
        colors[k] = colorRGB;
      }
    }
  ...
  const palette = {
    ...
    paneHeaderText: colors.isDarkTheme ? colors.dark : colors.light,
    ...
    facetBackgroundActive: _lighten(colors, colors.base, 0.7),
  ...
  // Add basic colors and overrided colors.
  Object.keys(colors).forEach((k) => {
    palette[k] = colors[k];
  });
